...
const render = (vdom, parent = null) => {
    ...
    if (isTextVdom(vdom)) {     //纯文本vdom直接渲染
        return mount(document.createTextNode(vdom));
    } else if (isElementVdom(vdom)) {   //html元素vdom渲染，需要渲染其子vdom并设置属性
        ...
        return dom;
    } else if (isComponentVdom(vdom)) {     //组件vdom调用renderComponent渲染
...
function renderComponent(vdom, parent) {
    ...
    if (Component.isPrototypeOf(vdom.type)) {       //继承自Component的组件
...
function patch(dom, vdom, parent = dom.parentNode) {
    ...
    if(isComponentVdom(vdom)) {     //组件vdom
        // 更新props和children
            ...
            if(typeof this[key] === 'function'){
                ...
                let newFunc = function () {
                    ...
                    Object.keys(this.ref).forEach(key => {
                        ...
                        if(ele.nodeName === 'INPUT'){   //input保留value
                            ref[key] = {value:ele.value};
                        }
                        else if(ele.nodeName === 'SELECT'){ //select保留value
                            ref[key] = {value:ele.value};
                        }
                        else if(ele.nodeName === 'BUTTON'){ //button保留innerText
                            ref[key] = {innerText:ele.innerText};
                        }
                    });
